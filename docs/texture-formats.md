...
**Lanczos2 LUT** is a sampled-only texture and so is unaffected by the storage
restrictions, but its natural format `r16snorm` does not exist at all in
baseline WebGPU — in wgpu it requires the `TEXTURE_FORMAT_16BIT_NORM` device
feature. Because that feature is orthogonal to the storage capabilities the
profiles classify, the LUT format is keyed on the feature rather than the
profile: `r16snorm` when the feature is enabled, otherwise it widens to
filterable `r16float` (the upload path converts the snorm values).
